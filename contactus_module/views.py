...
def user_register(request):
    if request.method == 'POST':
        ...
        if form_register.is_valid():
            date = form_register.cleaned_data
            User.objects.create_user(username=date['user_name'], email=date['email'], first_name=date['first_name'],
                                     last_name=date['last_name'], password=date['password_user2'])
